...
# Are parametric analyses on p_want feasible?
```{r compute number of unique datapoints}

#### count number of distinct p_want values by id and condition ####
distinct <- beh %>%
  group_by(id, cond) %>%
  summarize(distinct = n_distinct(p_want_of_resp))
...
distinct_PS_food <- beh[!(beh$cond=="OfficeLarge"| beh$cond=="OfficeSmall"),]  %>%
  group_by(id, portion_size) %>%
  summarize(distinct = n_distinct(p_want_of_resp))
...
#### count number of subjects with no variability ####
# likely cant do parametric analyses if only 1 value for a condition across runs
# note: to get a list of which subjects have no variability, %>% remove nrow()

# 23 subjects have no variability (distinct==1) at least 1 condition when assessing all cue conditions
distinct[(distinct$distinct==1),] %>%
  summarize(distinct = n_distinct(id)) %>% nrow()

# 11 subjects have no variability (distinct==1) at least 1 condition when assessing food conditions
distinct_food<-distinct[!(distinct$cond=="OfficeLarge" | distinct$cond=="OfficeSmall"),]
distinct_food[(distinct_food$distinct==1),] %>%
  summarize(distinct = n_distinct(id)) %>% nrow()

# 4 subjects have no variability (distinct==1) in at least 1 condition when assessing by ED
distinct_ED_food<-distinct_ED[!(distinct_ED$cue_type=="Office"),]
distinct_ED_food[(distinct_ED_food$distinct==1),] %>%
  summarize(distinct = n_distinct(id)) %>% nrow()

# 2 subjects have no variability (distinct==1) in at least 1 condition when assessing by PS (food only)
distinct_PS_food[(distinct_PS$distinct==1),] %>%
  summarize(distinct = n_distinct(id)) %>% nrow()
