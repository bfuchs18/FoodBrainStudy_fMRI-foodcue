...
beh_all$cue_type <- ifelse(grepl("High", beh_all$cond), "HighED", 
...
## datasets to use:
#beh_food_analyzed_PS
#beh_food_analyzed_ED
...
beh_pwant_HighED <- beh_food_analyzed_ED[beh_food_analyzed_ED$cue_type == 'HighED',]
beh_pwant_LowED <- beh_food_analyzed_ED[beh_food_analyzed_ED$cue_type == 'LowED',]
beh_pwant_LargePS <- beh_food_analyzed_PS[beh_food_analyzed_PS$portion_size == 'Large',]
beh_pwant_SmallPS <- beh_food_analyzed_PS[beh_food_analyzed_PS$portion_size == 'Small',]
...
p_want_mixed = lmer(p_want_of_resp ~ portion_size*cue_type + run + (1 | id), data = beh_food_clean)
anova(p_want_mixed)

# get predicted response after adjusting for run
beh_food_clean$predict_p_want <- predict(p_want_mixed, type="response")

#plot with ggplot
ggplot(data=beh_food_clean, aes(x=cue_type, y=predict_p_want, fill=portion_size)) +
  geom_bar(stat="identity", position=position_dodge())+
  scale_fill_brewer(palette="Paired")+
  theme_minimal()

# violin plot 
plot <- ggplot(beh_food_clean, aes(x=cue_type, y=predict_p_want, fill = portion_size)) +
  geom_violin(trim=FALSE)+
 # geom_signif(stat = "identity", inherit.aes = FALSE,
   #            data = data.frame(x = c(1.7, 0.7, 2.7), xend = c(3.3, 3.3, 3.3), y = c(105, 110, 85),  annotation = c("***", " ***", "*")),
  #             aes(x = x, xend = xend, y = y, yend = y, annotation = annotation)) +
...
# barplot with alaina's function
ps_by_ed <- data.frame(beh_food_clean$portion_size, beh_food_clean$cue_type)
mean_cond = means.function.na(beh_food_clean, beh_food_clean$p_want_of_resp, ps_by_ed) # get mean ps and ed
se_cond = se.function.na(beh_food_clean, beh_food_clean$p_want_of_resp, ps_by_ed)

beh_food_clean$portion_size <- as.factor(beh_food_clean$portion_size)
bar_graph.se(means = mean_cond, er = se_cond, xlab = "", ylab = "", ymax = 1, ymin = 0, group = beh_food_clean$portion_size)
