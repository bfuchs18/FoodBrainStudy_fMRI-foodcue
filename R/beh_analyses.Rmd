...
```{r post-scan rating violin plots}
#### average fullness ratings ####
# wide to long
full_ratings_long <- melt(avg_ratings,
        # ID variables - all the variables to keep but not split apart on
    id.vars=c("id"),
        # The source columns
    measure.vars=c("full_HighLarge","full_LowLarge", "full_Large", "full_HighSmall", "full_LowSmall", "full_Small"),
...
# plot
ggplot(full_ratings_long, aes(x = condition, y = avg_fullness)) + 
...
#### large and small liking ratings #### 
# wide to long
like_ratings_long <- melt(avg_ratings,
        # ID variables - all the variables to keep but not split apart on
    id.vars=c("id"),
        # The source columns
    measure.vars=c("like_LargePS","like_SmallPS"),
        # Name of the destination column that will identify the original
        # column that the measurement came from
    variable.name="condition",
...
# plot
ggplot(like_ratings_long, aes(x = condition, y = avg_liking)) + 
  geom_violin(aes(fill = condition), trim = FALSE) +
  geom_boxplot(width = 0.1) +
  theme_classic() +
  geom_dotplot(binaxis = "y",
...
like_long <- reshape2::melt(analyzed_df,
        # ID variables - all the variables to keep but not split apart on
    id.vars=c("id"),
        # The source columns
    measure.vars=c("like_HighLarge","like_HighSmall", "like_LowLarge", "like_LowSmall"),
        # Name of the destination column that will identify the original
        # column that the measurement came from
    variable.name="condition",
    value.name="avg_liking"
...
full_long <- reshape2::melt(analyzed_df,
        # ID variables - all the variables to keep but not split apart on
    id.vars=c("id"),
        # The source columns
...
like_long$ed <- ifelse(grepl("High", like_long$cond), "High", "Low")
full_long$portion_size <- ifelse(grepl("Large", full_long$cond), "Large", "Small")
full_long$ed <- ifelse(grepl("High", full_long$cond), "High", "Low")

# boxplot
ggplot(like_long, aes(x=portion_size, y=avg_liking, fill=ed)) +
  geom_boxplot() 
ggplot(full_long, aes(x=portion_size, y=avg_fullness, fill=ed)) +
  geom_boxplot() 

# descriptive stats 
like_long %>%
  group_by(portion_size, ed) %>%
  get_summary_stats(avg_liking, type = "mean_sd")

full_long %>%
  group_by(portion_size, ed) %>%
  get_summary_stats(avg_fullness, type = "mean_sd")

# repeated measures anovas (ed * ps)
liking.aov <- anova_test(
  data = like_long, dv = avg_liking, wid = id,
  within = c(ed, portion_size)
  )
