...
#### ANALYSES ####
# portion size (large, small) x category (high, low, office) mixed model
p_want_mixed_all = lmer(p_want_of_resp ~ portion_size*category + run + (1 | id), data = beh_clean)
...
# portion size (large, small) x food-cue type (high, low)
p_want_mixed_food = lmer(p_want_of_resp ~ portion_size*energy_density + run + (1 | id), data = beh_food_clean)
anova(p_want_mixed_food)

# get predicted response after adjusting for run
beh_food_clean$predict_p_want <- predict(p_want_mixed_food, type="response")
