...
beh_food_clean <- beh_food[ ! beh_food$id %in% missing_pwant$id, ]
...
# 2 subjects (id 40, 58) have no variability (distinct==1) in at least 1 condition when assessing by ED
novar_ED <- distict_ED[(distict_ED$distinct==1),] %>%
  summarize(distinct = n_distinct(id))
novar_ED$id
nrow(novar_ED)

# 1 subject (id 40) has no variability (distinct==1) in at least 1 condition when assessing by PS
