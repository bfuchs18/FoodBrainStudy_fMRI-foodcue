...
beh <- read.delim("data/databases/task-foodcue_summary.tsv")
image_ratings <- read.csv("data/compiled/fmri_image_ratings.csv")
...
```{r avg wanting: descriptives and plot, include=TRUE}
...
```{r trying to graph behavior by run}


#goal: line plot of wanting by run (x-axis) for each condition, 1 line per subject 
## i.e. -- each subject has 1 line per condition (condition is indicated by color)

beh_1 <- beh[beh$id == "1", c("id", "cond", "p_want_of_resp", "run")]
beh_2 <- beh[beh$id == "2", c("id", "cond", "p_want_of_resp", "run")]
beh_12 <- beh[(beh$id == "1" | beh$id == "2"), c("id", "cond", "p_want_of_resp", "run")]
...
beh_HL <- beh[beh$cond == "HighLarge", c("id", "cond", "p_want_of_resp", "run")]


ggplot(id, aes(x = year, y = health_exp_total, color = cond)) +
  geom_line(aes(group = id)) +
  geom_line(data = gd)

###~~~~~~~~~~~
# https://psyteachr.github.io/introdataviz/multi-part-plots.html
beh_HL <- beh[(beh$cond == "HighLarge"), c("id", "cond", "p_want_of_resp", "run")]
beh_LL <- beh[(beh$cond == "LowLarge"), c("id", "cond", "p_want_of_resp", "run")]
beh_Large <- beh[(beh$cond == "HighLarge" | beh$cond == "LowLarge"), c("id", "cond", "p_want_of_resp", "run")]

ggplot(beh, aes(x = run, y = p_want_of_resp, 
                     group = cond, shape = cond)) +
  # adds raw data points in each condition
  geom_point(aes(colour = cond),alpha = .2) +
  # add lines to connect each participant's data points across conditions
  geom_line(aes(group = id, colour = cond), alpha = .2) +
...
# looks as expected using beh_HL
ggplot(beh_HL, aes(x = run, y = p_want_of_resp, 
                     group = cond, shape = cond)) +
  # adds raw data points in each condition
  geom_point(aes(colour = cond),alpha = .2) +
  # add lines to connect each participant's data points across conditions
...
ggplot(beh_LL, aes(x = run, y = p_want_of_resp, 
                     group = cond, shape = cond)) +
  # adds raw data points in each condition
  geom_point(aes(colour = cond),alpha = .2) +
  # add lines to connect each participant's data points across conditions
...
### THIS WORKS BETTER??? Colors are confusing bc of overlap 
IDCondition <- paste(beh$id, beh$cond)
ggplot(beh, aes(x=run, y=p_want_of_resp, group=IDCondition)) +
  geom_point(size=.5, aes(colour=cond), alpha=0.3)+
  geom_line(size=1, alpha=0.05, aes(colour = cond))+
...
IDCondition_food <- paste(beh_food_NAs$id, beh_food_NAs$cond)

ggplot(beh_food_NAs, aes(x=run, y=p_want_of_resp, group=IDCondition_food)) +
  geom_point(size=.5, aes(colour=cond), alpha=0.3)+
  geom_line(size=1, alpha=0.5, aes(colour = cond))+ #alpha is opacity
