...
# V6_compiled contains demographic MRI-related variables for children who attended visit 6
source("R/data_org.R")
...
# Create 'compiled' dataframe with pre-MRI fullness variables
...
# frequency of maternal BMI method
table(attended_df$mom_bmi_method)
table(analyzed_df$mom_bmi_method)
```

#### Demographics
```{r demographics table by inclusion status}
...
FullTable1(
  data = attended_df, vars = c("sex", "age_yr", "income_recode","mom_edu_recode", "bmi_percentile", "fmi"),
  var_names = c("sex","age (years)", "family income", "Maternal Education", "BMI percentile", "Fat mass index"),
...
label(analyzed_df$mom_bmi_method) <- "Maternal anthropometrics"


# # function to round percentages to whole number --- does not round for missing data
# my.render.cat <- function(x) {
#     c("", sapply(stats.default(x), function(y) with(y,
#         sprintf("%d (%0.f%%)", FREQ, PCT))))
# }

# generate table: high vs. low risk -- dont included 'overall' since that info is in the previous table
table1(~ sex + age_yr + race + ethnicity + income_recode + mom_edu_recode + bmi_percentile + mom_bmi_method| risk_status_mom, data=analyzed_df)
...
FullTable1(
  ...
  strata = "risk_status_mom", stars = "none", p_col = FALSE,
  html = c(TRUE) # for html table
)
```


```{r compare demo: high vs low}
...
# generate table: high vs. low risk -- dont included 'overall' since that info is in the previous table
table1(~ age_yr + bmi_percentile + fmi + sex +  + race + ethnicity + income_recode + mom_edu_recode + n_run_good + n_vol_goodruns + fd_avg_allruns + pre_mri_ff + cams_pre_mri + pre_mri_snack | risk_status_mom, data=analyzed_df, render.continuous=my.render.cont)
#       render.categorical=my.render.cat)
...
# generate table 
table1(~ n_run_good + n_vol_goodruns + fd_avg_allruns + pre_mri_ff + pre_mri_snack + cams_pre_mri | risk_status_mom ,data=analyzed_df)
