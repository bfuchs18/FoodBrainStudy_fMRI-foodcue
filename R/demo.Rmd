...
censor_sum <- read.csv("data/compiled/censor_sum.csv")
...
TR_count <- censor_sum_goodruns %>% group_by(id) %>% summarise(n_vol_goodruns = sum(n_vol_include))

# add TR_count to compiled
compiled<-merge(x=compiled,y=TR_count,by="id",all.x=TRUE)


# response rate in scanner?? 
```
...
# generate table 
table1(~ n_run_good + n_vol_goodruns + fd_avg_allruns + pre_mri_ff + cams_pre_mri, data=analyzed_df)
